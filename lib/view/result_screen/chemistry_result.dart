...
class ChemistryResult extends StatefulWidget {
  const ChemistryResult({required this.rightAnswerCount, super.key});
  final int rightAnswerCount;

  @override
  State<ChemistryResult> createState() => _ChemistryResultState();
}

class _ChemistryResultState extends State<ChemistryResult> {
  int starCount = 0;
  late int yourScore;

  calculateStarccount() {
    var percentage =
        widget.rightAnswerCount / Dummydb.chemistryQuestionList.length * 100;
    if (percentage >= 80) {
      starCount = 3;
